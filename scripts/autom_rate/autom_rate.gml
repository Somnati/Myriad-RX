...
function autom_rate(_kind) {
	...
	if (variable_global_exists("offline_replaying") && g.offline_replaying) {
		...
		switch (_kind) {
			...
			case "fab":   return (_a.fab.on ? (_r.fab   / 100) * (1 + sprite_staff("fab")) : 0) * cheat_rate("fab") * deck_tile_rate("fab");
			case "merge": return (_r.merge / 100) * (1 + sprite_staff("merge")) * cheat_rate("merge") * deck_tile_rate("merge");
		}
		return 1;
	}
	...
	switch (_kind) {
		...
		case "fab":   return (_a.fab.on ? (_a.fab.spd / 100) * ram_throttle() : 0) * cheat_rate("fab") * deck_tile_rate("fab");
		case "merge": return (_a.am_speed / 100) * ram_throttle() * cheat_rate("merge") * deck_tile_rate("merge");
	}
	return 1;
}

...
function region_ledger(_d, _ri, _tab = "lanes") {
	...
	switch (_tab) {
	case "lanes": {
		array_push(_out, { k : "", v : "six moods of the region, each at rest at 0. what the crews do here pushes one, and left alone it drifts back to rest over days. green is good for the region, orange bad.", col : sett_ink });
		var _w = region_weight(_rg), _hl = ln(2) * LANE_TAU / _w / 3600;
		array_push(_out, { k : "weight", v : string_format(_w, 1, 2) + "  -  every push is divided by it (a bigger, busier region shrugs more off); a push halves back toward rest every " + string_format(_hl, 1, 1) + "h of expedition time", col : sett_ink });
		...
		for (var _j = 0; _j < array_length(_nm); _j++) {
			...
			array_push(_out, { k : _nm[_j], v : ((_v >= 0) ? "+" : "") + string_format(_v, 1, 2) + "  " + _word, col : (abs(_v) < .05) ? sett_ink : (_isbad ? c_horange : c_seagreen), bar : _v,
			                   sub : "pushed by: " + _push[$ _nm[_j]] + ".  shows up as: " + _reads[$ _nm[_j]] + "." });
		}
		...
		array_push(_out, { k : "scars", v : (_srow != "") ? _srow : "none yet", col : (_srow != "") ? c_gold : sett_ink,
		                   sub : "hold order or trade past +.7, or faith under -.7, for " + string(round(SCAR_HOLD / 3600)) + "h and the region changes for good: a camp becomes a settlement, a settlement a village, a village a town, or the dead spread. three a region at most." });
	} break;
	case "factions": {
		...
		array_push(_out, { k : "shows up as", v : "which kinds turn up in a fight (a thin kind less, the others more); a camp's count; the leader's guards (none under 50%) and his stats (" + string_format(1 - FAC_BOSS_CUT, 1, 1) + " + " + string_format(FAC_BOSS_CUT, 1, 1) + " x strength); the road - thin bandits become passers-by, thin beasts a sprite met.", col : sett_ink });
	} break;
	case "people": {
		array_push(_out, { k : "", v : "how many live at a place: its baseline, a slow wobble of up to " + string(round(POP_WOBBLE * 100)) + "% either way, and what happened lately - a raid -" + string(round(RAID_POP * 100)) + "%, rats -8%, a fair +5%, a wave held +5%, the villain ended +4% - which fades by the place's size - a city in 3 days, a town 5, a village 7, a settlement 10. a settlement stays a settlement: only a scar changes what a place is.", col : sett_ink });
		var _pany = false;
		for (var _pi = 1; _pi < array_length(_rg.nodes); _pi++) {
			...
			_psub += ".  its shop stocks one item fewer under -12%, one more over +10%";
	...
	case "memory": {
		array_push(_out, { k : "", v : "what one place remembers for a while - a camp burned, a town helped, an inn that threw the crew out - each fading on the expedition clock. the lanes are the whole region's mood; these are one place's.", col : sett_ink });

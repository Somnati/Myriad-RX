...
function galaxy_sky_build(_dw = undefined) {
	...
	for (var _i = 0; _i < _n; _i++) {
		...
		array_push(_out.stars, { x : dcos(_el) * dcos(_az), y : -dsin(_el), z : dcos(_el) * dsin(_az),
		                         ...
		                         skind : (_st.props[$ "skind"] ?? "main"), sspin : (_st.props[$ "spin"] ?? 1), sseed : _st.seed });   // (ph: the twinkle's phase; near: a real neighbour - a halo when big, 2026-09-16; skind: its kind shows from here too, 2026-09-17)
	}

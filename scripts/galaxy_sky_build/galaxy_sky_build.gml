...
function galaxy_sky_build(_dw = undefined) {
	...
	for (var _i = 0; _i < _n; _i++) {
		...
		array_push(_out.stars, { x : dcos(_el) * dcos(_az), y : -dsin(_el), z : dcos(_el) * dsin(_az),
		                         col : _st.props.color, b : (.2 + .45 * _ld) + (.8 - .45 * _ld) * clamp(_apb, 0, 1), s : clamp(1 + 30 * _ap, 1, _cfg[$ "sky_size_max"] ?? 31), ph : (_hh mod 360), near : true });   // (ph: the twinkle's phase; near: a real neighbour - a halo when big, 2026-09-16)
	}

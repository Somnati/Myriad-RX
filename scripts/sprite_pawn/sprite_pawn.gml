...
function sprite_pawn(_sp, _hp = undefined, _mpf = undefined) {
	...
	// THE ABILITIES (2026-09-17, the evilities): the worn four's lanes. The
	// STAT lanes (hp / atk / def / int / res / spd / hit) are already in
	// _p - sprite_stats folds them, so the sheet and the pawn agree (his
	// report: the sheet's def never moved); here the rest - points of
	...
	return {
		...
		atk : _p.atk, def : _p.def, mag : _p.mag, mdef : _p.mdef,
		spd : _p.spd, hit : _p.hit,

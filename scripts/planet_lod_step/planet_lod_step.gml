	...
	while (_l.row < _l.h && get_timer() < _until) {
		...
		for (var _i = _l.col; _i < _w; _i++) {
			...
			if (_hascone) {
				...
				if (_cf > 0 && _cf < VOLCANO_BARE) _ps.oe = lerp(_ep[_i00] * _w00 + _ep[_i10] * _w10 + _ep[_i01] * _w01 + _ep[_i11] * _w11, _ps.oe, clamp((_cf - (VOLCANO_BARE - .07)) / .07, 0, 1));   // (the map's own hand-over at VOLCANO_BARE - a soft edge of it; the old lerp lifted the whole flank halfway, q276)
			}

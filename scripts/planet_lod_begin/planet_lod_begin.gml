...
function planet_lod_begin(_pn, _k) {
	...
	var _l = { k : _k, w : _w, h : _h, row : 0, col : 0, tbuf : buffer_create(_w * _h * 4, buffer_fixed, 1), hbuf : buffer_create(_w * _h * 4, buffer_fixed, 1), tsurf : -1, hsurf : -1, ready : false,
		u0 : 0, v0 : 0, uw : _pn.tw, vh : _pn.th,   // (the window planet_draw hands the shader: the whole map)
		order : [], focus_v : .5, partial : false, upl : 0 };   // (the rows' build order by focus, the partial uploads' mark - q270)
	planet_lod_focus(_l, .5);
	return _l;
}

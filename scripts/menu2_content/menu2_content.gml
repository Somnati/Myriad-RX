...
function menu2_content() {
	...
	if (unfold_has("rebirth"))    menu2_button("rebirth",      function() { rebirth_open(); }, c_hred); // myriad
	if (unfold_has("upgrades"))   menu2_button("upgrades",     function() { upgrades_open(); }, c_lavender); // myriad (an overlay, 2026-09-12)
	if (unfold_has("tiles"))      menu2_button("tiles",        function() { tiles_open(); },     c_aqua);        // myriad (an overlay, 2026-09-12)
	if (unfold_has("abilities"))  menu2_button("abilities",    function() { abilities_open(); }, c_rarity_epic); // techdemo (an overlay, 2026-09-12)
	if (unfold_has("automation")) menu2_button("automation",   function() { automation_open(); }, c_sblue);
	if (unfold_has("timebank"))   menu2_button("time bank",    function() { timebank_open(); }, c_gold);
	if (unfold_has("battery"))    menu2_button("battery",      function() { battery_open(); },  c_sgreen);   // the offline budget + the crank
	if (unfold_has("ccore"))      menu2_button("credit core",  function() { ccore_open(); },    c_lavender); // DE's credit farm, the well of credits
	...
	if (unfold_has("statistics")) menu2_button("statistics",   function() { statistics_open(); }, c_sgreen);
	...
	if (unfold_has("offlog"))      menu2_button("offline log",  function() { offlog_open(); }, c_sgreen);   // every absence's story (2026-09-12)
	if (unfold_has("expeditions")) menu2_button("expeditions",  function() { exped_open(); }, c_steelblue);   // the mock (2026-09-12)
}

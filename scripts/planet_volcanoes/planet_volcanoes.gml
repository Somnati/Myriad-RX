...
/// A terra world has none two times in five, else one to three; a lava
/// world three to seven, every one live; a barren world one to three,
/// all dead. Hashed off the seed - nothing rolled. A cone is a lift by
...
/// round each live vent a disc of smoke in the cloud map's GREEN
...
function planet_volcanoes(_pn) {
		...
		cone : function(_x, _y, _rad, _hgt, _live) {
			...
			for (var _dy = -_r; _dy <= _r; _dy++) {
				...
				for (var _dx = -_rx; _dx <= _rx; _dx++) {
					...
					if (_d > _rad) continue;
					var _l = _hgt * power(1 - _d / _rad, 1.25);
		...
		plume : function(_x, _y, _rad) {
			...
			for (var _dy = -_r; _dy <= _r; _dy++) {
				...
				for (var _dx = -_rx; _dx <= _rx; _dx++) {
					...
					if (_d > 1) continue;
					var _a = (_d < .55) ? 1 : (1 - (_d - .55) / .45);
					_a *= .85 + .3 * dt[_i];   // (ragged)
					if (_d > .7 && dt[_i] < .35) _a *= .5;
	...
	if (_arch == "lava")        { _nv = 3 + (_hv mod 5); _all_live = true; }
	else if (_arch == "barren") { _nv = 1 + (_hv mod 3); _none_live = true; }
	else                        { _nv = (_hv < 40) ? 0 : ((_hv < 75) ? 1 : ((_hv < 95) ? 2 : 3)); }
	...
	for (var _k = 0; _k < _nv; _k++) {
		...
		var _rad = (3 + 3 * _c.h(_b + 91)) * _sc, _hgt = .18 + .14 * _c.h(_b + 92);
	...
	for (var _k = 0; _k < array_length(_vents); _k++) {
		var _vt = _vents[_k];
		if (_vt[2]) { var _nf = 1 + (hash_mix(_pn.seed, 31500 + _k) mod 3); for (var _f = 0; _f < _nf; _f++) _c.flow(_vt[0], _vt[1], round((4 + 8 * _c.h(_vt[4] + 50 + _f)) * _sc), _vt[4] + 60 + _f * 200); }
	}
	var _vent = _c.vent;
	for (var _i = 0; _i < _n; _i++) {
	...
	for (var _k = 0; _k < array_length(_vents); _k++) { var _vt = _vents[_k]; if (_vt[2]) _c.plume(_vt[0], _vt[1], _vt[3] * 1.3); }
}

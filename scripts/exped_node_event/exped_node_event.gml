	...
	if (_qhere) {
		switch (_q.kind) {
			...
			case "escort":
				...
				else { _q.done = _q.n; array_push(_tr.log, _q.who + " delivered to " + _nd.name + ", cart and all. paid on the nail. the quest is done"); _tr.act = { kind : "look", left : EXPED_ROOM_T * .5, steps : 1 }; }
				break;
			case "fetch":
				...
				else { _q.done = _q.n; array_push(_tr.log, "handed " + _q.who + " over in " + _nd.name + ". " + choose("nobody said thank you", "there was a small fuss", "it was the wrong one, but they kept it") + ". the quest is done"); _tr.act = { kind : "look", left : EXPED_ROOM_T * .5, steps : 1 }; }
				break;
			case "rescue":
				...
				else { _q.done = _q.n; array_push(_tr.log, _q.who + " home in " + _nd.name + ", to some fuss. the quest is done"); _tr.act = { kind : "look", left : EXPED_ROOM_T * .5, steps : 1 }; }
				break;

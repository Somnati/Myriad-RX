...
function exped_shop(_tr) {
	...
	var _bought = 0, _pl = sprite_personalities();
	for (var _k = 0; _k < array_length(_tr.sids); _k++) {
		...
		for (var _s = 0; _s < array_length(_stock); _s++) {
			if (_stock[_s].sold) continue;
			var _it = _stock[_s].it, _slot = _it.slot;
		...
		var _st = _stock[_best], _it2 = _st.it, _price = max(1, _st.price + _hag);
		if (_tr.credits >= _price) {
			...
			array_push(_tr.log, _sp.name + " looked at " + _it2.name + " (" + string(_price) + " credits) and " + choose("walked out", "put it back", "decided against it", "could not afford it", "counted the pocket twice and left", "asked " + _keeper + " to hold it. " + _keeper + " will not", "said \"next time\" to it, and meant it", "left a fingerprint on it and nothing else"));
		}
	}
	if (_bought == 0 && roll_perc(50)) array_push(_tr.log, choose("the shops of " + _nd.name + " had nothing worth the walk", _sign + " had nothing for them, and " + _keeper + " said so", "nothing bought in " + _nd.name + ". " + _keeper + " watched them go", "the shelf was looked at. the shelf was left", "the shop's best thing was the cat, which was not for sale"));
	else if (_bought > 0 && roll_perc(30)) array_push(_tr.log, _keeper + " " + choose("wrapped it in paper that had been used before", "said it was the last one. it was not", "threw in a piece of string", "bit the coin, out of habit", "said \"come back\", in a voice that did not care either way"));
}

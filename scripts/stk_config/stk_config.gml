/// @description stk_config() -> THE STACK's roster (q316): three layers, seven sinks each (the seventh behind a cinder perk) - declarative, APPEND ONLY (the index is the save format and the wiring)
/// A sink: { key, name, col, per (bonus % a level^0.75), kind, what, [thk] }. thk = the sink's threshold factor (the wells are DEEP: x14 / x4 the ladder).
...
///   q317 the seventh: "siphon" - its energy feeds aether's working sinks (stk_tick), the bonus multiplies the feed
///                     "mirror" - multiplies the focused aether sink's effect (stk_mult)      "cinders" - multiplies the turn's cinders (stk_cinders)
function stk_config() {
	static _c = [
		[   // ENERGY
			...
			{ key : "well",      name : "the well",    col : c_hpurple,   per : 12,  kind : "cap_ae",      what : "aether cap", thk : 14 },
			...
			{ key : "siphon",    name : "siphon",      col : c_hpurple,   per : 20,  kind : "siphon",      what : "aether feed" },
		],
		[   // AETHER
			...
			{ key : "mirror",    name : "mirror",      col : c_white,     per : 12,  kind : "mirror",      what : "the focused sink" },
		],
		[   // QUINTESSENCE

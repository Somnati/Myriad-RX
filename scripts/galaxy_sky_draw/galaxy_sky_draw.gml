...
function galaxy_sky_draw(_sky, _cam, _cx, _cy, _w, _h, _sun = true, _sibs = true, _occ = undefined) {
	...
	if (_skd0 == "swell") _ss *= _slt.pulse;   // (the sun swells and snaps - q267)
	...
	for (var _i = 0; _i < array_length(_stars); _i++) {
		...
		if ((_sk[$ "skind"] ?? "main") == "swell") _a *= .75 + .35 * (star_swell(_sk[$ "sseed"] ?? 0).s - .84) / .58;   // (a swelling neighbour brightens as it fills - q267)   // (a cloud point is grain: its raw brightness, not the stars' floor - the band stayed as he liked it)
	...
	if (_sun && _sun_on) {
		...
		if (_skd1 == "chroma") chroma_draw(_ssx, _ssy, 5.5 * _ss, _sky.sun_col, (_sky[$ "star"] ?? 0) * .37, _sfade, _cam);   // (the chromatic sun - q267)
		else if (_skd1 == "swell") swell_draw(_ssx, _ssy, 5.5 * _ss / max(.5, _slt.pulse), _sky.sun_col, _sky[$ "sseed"] ?? 0, _sfade, _cam);   // (the swelling sun: swell_draw sizes it itself - q267)
		else star_draw(_ssx, _ssy, 5.5 * _ss, _sky.sun_col, (_sky[$ "star"] ?? 0) * .37, _sfade, _cam);

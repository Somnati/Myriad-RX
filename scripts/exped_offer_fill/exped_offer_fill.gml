...
function exped_offer_fill(_of) {
	...
	if (is_array(_of[$ "pq"])) for (var _i = 0; _i < array_length(_of.pq); _i++) {
		...
		var _q = { kind : _r.kind, node : clamp(_r.node, 0, _nn - 1), from : (_r.from < 0) ? -1 : clamp(_r.from, 0, _nn - 1), foe : _r.foe, n : _r.n, mult : _r.mult, who : _r.who, pers : 1, pnote : _r.pnote, vil : _r[$ "vil"] ?? 0 };
		if (is_array(_r[$ "nodes"]) && array_length(_r.nodes) > 0) _q.nodes = _r.nodes;
		exped_quest_finish(_q, _rg, _of.ri, false);
		_q.reward = ceil(_q.reward * ((_q.vil >= 3) ? 2 : 1.5));   // (the villain's boss pays double)
		_p.q = _q;
	}
}

...
function handle_save(){
	...
	var _xrv = handle("ex_regv", (action == sv_save) ? 3 : 1);   // THE REGION LAW'S VERSION (q287 / q288): 3 = places on the ground; an older save drops what its regions held
	...
	if (action == sv_load) {
		...
		exped_unpack(_xt);
		if (_xrv < 3) exped_regions_reset();   // THE REGION BREAK (q287 / q288, his call): an older save's trips come home, its region records go
		exped_board_roll();
	}

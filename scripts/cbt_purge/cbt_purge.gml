...
function cbt_purge(_f, _t, _school, _one = false) {
	...
	if (_school == "light") {
		...
		if (is_struct(_t[$ "nf"])) { if (_t.nf.atk > 0) { _t.nf.atk = 0; _n++; } if (_t.nf.def > 0) { _t.nf.def = 0; _n++; } if (_t.nf.hit > 0) { _t.nf.hit = 0; _n++; } }
		if (_n > 0) { cbt_log(_f, _t.name + " is cleansed"); cbt_film(_f, _t, 0, _t.name + " is cleansed"); }
		return _n;
	}
	if (_school == "dark") {
		if (is_struct(_t[$ "bf"])) { if (_t.bf.atk > 0) { _t.bf.atk = 0; _n++; } if (_t.bf.def > 0) { _t.bf.def = 0; _n++; } if (_t.bf.hit > 0) { _t.bf.hit = 0; _n++; } if (_t.bf.spd > 0) { _t.bf.spd = 0; _n++; } }

...
function exped_quest_target(_q) {
	...
	switch (_q.kind) {
		case "escort": case "fetch": case "rescue": case "parcel": case "goat": return ((_q[$ "at"] ?? 0) == 0 && (_q[$ "from"] ?? -1) >= 0) ? _q.from : _q.node;
		case "survey": case "count": { var _ns = _q[$ "nodes"]; if (is_array(_ns) && array_length(_ns) > 0) return _ns[clamp(_q.done, 0, array_length(_ns) - 1)]; break; }
	}
	return _q.node;
}

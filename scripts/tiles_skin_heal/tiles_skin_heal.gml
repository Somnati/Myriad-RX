...
/// the board: as long as the slots, 0 wherever the slot is empty, and a
/// fresh roll wherever a tile has none (-1: a save from before the
/// materials, a site that forgot to roll). Called from tiles_sync (every
...
function tiles_skin_heal() {
	...
	for (var _i = 0; _i < _t.slots; _i++) {
		...
		if (is_undefined(_s) || !is_real(_s) || _s < 0) _t.skin[_i] = tile_skin_roll(_t.tier[_i]);
	}
}

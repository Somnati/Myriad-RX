...
if (copy_flash > 0) copy_flash -= delta;

// ---- live search capture (keyboard_string while the find box is lit) ----
if (search_on) {
	var _q = string_copy(keyboard_string, 1, 20);
	if (_q != search) { search = _q; rebuild = true; }
...
if (mouse_check_button_pressed(mb_left)) {
	...
	if (point_in_rectangle(mouse_x, mouse_y, room_width - 62,
		...
		if (search_on && os_type == os_android) keyboard_virtual_hide();
		back_room();
		exit;
	}

	// find: toggles live search (the title becomes the query line)
	if (point_in_rectangle(mouse_x, mouse_y, room_width - 106,
		...
		if (os_type == os_android) {
			if (search_on) keyboard_virtual_show(kbv_type_default,
				kbv_returnkey_search, kbv_autocapitalize_none, false);
			else keyboard_virtual_hide();
		}
		rebuild = true;
		play_sound_ext(snd_softclick, search_on ? 1.1 : .9, search_on ? 1.2 : .9, .4, 1);
	}

	// copy: the WHOLE tree (open or not) to the clipboard as text
	if (point_in_rectangle(mouse_x, mouse_y, room_width - 150,
		_bby + 6, room_width - 110, _bby + 22)) {
		var _txt = "";
		for (var _d = 0; _d < array_length(dump); _d++) {
			_txt += string_repeat(" ", dump[_d].dep * 2) + dump[_d].name;
			if (dump[_d].val != "") _txt += ": " + dump[_d].val;
			_txt += "\n";
		}
		clipboard_set(_txt);
		copy_flash = 90;
		play_sound_ext(snd_matclick2, 1.1, 1.2, .5, 1);
	}

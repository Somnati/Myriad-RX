...
tic -= delta;
// ⚖️ THE BURGER IS THE SETTINGS X TOO (his suggestion). The overlay
// covers the room from the header down and the burger is the one
// control still above it, so it becomes the way out rather than a
// second way in - and it wears the X while it means that, which is the
// morph it already had for its own drawer.
var _sett = instance_exists(syst_settings);
t = move_to(t, (open || _sett) ? 1 : 0, 4); // adj is a divisor (delta-aware)
...
if (hot && tic <= 0) {
	tic = 8;
	// settings up: this press CLOSES it and opens nothing. Two panels
	// over each other with one X between them is a guessing game.
	if (_sett) {
		settings_close();
		rip = 1;

...
__back = function() {
	...
	switch (view) {
		...
		case "planet": if (pv_mode == "region") { if (rg_leave) return; if (hand != "") __hand_fold(); rg_leave = true; rg_infl = false; pv_pop = undefined; } else { __sy_enter(galaxy_world_sys(pl_dest).star); sy_from = "galaxy"; __page_go("system"); } break;   // (the planet -> its star system, q270 - his ask; no back button closes the panel: the X does) region mode swings out -> the planet; the planet's [close] folds the panel (the hub went, 2026-09-16)

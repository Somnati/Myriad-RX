...
if (view == "trip") {
	...
	__draw_orbit(_d, big_x + 2, big_y + 1, big_w - 3, big_h - 2, (big_w - 3) * .5, (big_h - 2) * .5 + 2, land ? 22 : 26, tp_cam, tp_spin, _tr[$ "rgi"] ?? 0, _tr[$ "rgi"] ?? 0, 1);
	ui_fade_set(_ea);
	draw_set_halign(fa_center);
...
if (view == "galaxy") {
	...
	if (!shader_is_compiled(sh_galaxy_fog) || !shader_is_compiled(sh_sky_fog) || !shader_is_compiled(sh_nebula) || !shader_is_compiled(sh_sky_inside) || !shader_is_compiled(sh_star)) { draw_set_font(fnt); draw_set_halign(fa_left); draw_set_color(c_hred); draw_set_alpha(.95); draw_text(_gr.x + 4, _gr.y + _gr.h - 30, (shader_is_compiled(sh_galaxy_fog) ? "" : "sh_galaxy_fog failed to compile  ") + (shader_is_compiled(sh_sky_fog) ? "" : "sh_sky_fog failed to compile  ") + (shader_is_compiled(sh_nebula) ? "" : "sh_nebula failed to compile  ") + (shader_is_compiled(sh_sky_inside) ? "" : "sh_sky_inside failed to compile  ") + (shader_is_compiled(sh_star) ? "" : "sh_star failed to compile")); }   // (2026-09-16: the page is the compile log)
	ui_fade_set(_ea);

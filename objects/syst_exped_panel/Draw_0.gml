...
if (view == "galaxy") {
	...
	if (!shader_is_compiled(sh_galaxy_fog) || !shader_is_compiled(sh_sky_fog) || !shader_is_compiled(sh_nebula) || !shader_is_compiled(sh_sky_inside)) { draw_set_font(fnt); draw_set_halign(fa_left); draw_set_color(c_hred); draw_set_alpha(.95); draw_text(_gr.x + 4, _gr.y + _gr.h - 30, (shader_is_compiled(sh_galaxy_fog) ? "" : "sh_galaxy_fog failed to compile  ") + (shader_is_compiled(sh_sky_fog) ? "" : "sh_sky_fog failed to compile  ") + (shader_is_compiled(sh_nebula) ? "" : "sh_nebula failed to compile  ") + (shader_is_compiled(sh_sky_inside) ? "" : "sh_sky_inside failed to compile")); }   // (2026-09-16: the page is the compile log)
	ui_fade_set(_ea);

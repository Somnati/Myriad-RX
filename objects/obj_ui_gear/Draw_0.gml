...
// hover halo, the burger's exactly - the two icons are one control
// surface and should light the same way
if (hot || rip > 0) {
	var _gw = sprite_get_width(spr_vis_glow_soft);
	draw_sprite_ext(spr_vis_glow_soft, 0, _s.x, _s.y, 26 / _gw, 26 / _gw, 0,
		c_white, .1 + .12 * spin + .15 * rip);
}

// DE's cog is frame 0; frame 1 is its inverse (the cog cut out of a
// disc), which is the wrong read for a flat icon in a header.

...
/// its steps under it each behind a box - GREEN with a black check once
...
// ---- the plate: open, the dial dock's glass - the room under it
// pixelated (this frame's shot) under the dock's light dim (.45);
...
for (var _i = 0; _i < array_length(_ls); _i++) {
	...
	if (_e > 0) draw_sprite_ext(spr_pixel_1x1, 0, _bx + 1, _by + 1, 4 * _e, 4, 0, merge_colour(c_sgreen, c_hred, _rd), .95 * _al);
	if (_e >= .999) {
		// the check: four cells, black on the green
